...
describe('the teammate card registry', () => {
  it('covers the teammate card, your own health, your infected health, the ability timer, the infected card, the use bar and the spawn panel', () => {
    expect(PANEL_CHILDREN.map((p) => p.panelId)).toEqual(['teamColumn', 'ownHealth', 'siHealth', 'abilityRing', 'infectedRow', 'progressBar', 'ghostPanel']);
