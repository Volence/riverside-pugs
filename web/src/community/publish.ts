...
/**
 * The preview PNG, drawn off screen as the editor's canvas draws: the scene
 * backdrop, then the survivor side, Healthy, holding the gun, nothing
 * selected. Style images and fonts load as they are asked for, so it
 * repaints on each one and finishes once none has arrived for `quietMs`, or
 * at `maxMs` whatever is still loading.
 */
export async function renderPreview(design: HudDesign, o: { quietMs?: number; maxMs?: number } = {}): Promise<Blob> {
  ...
  return new Promise<Blob>((resolve, reject) => {
    ...
    let queued = false;
    const paint = () => {
      drawBackdrop(ctx, w, h, 'scene', null, null);
 ...
 * saves it; `import` is sent only for a design on an imported HUD, with its
 * id in meta.
 */
export function buildHudForm(o: {
  title: string; description: string; permission: boolean; prepared: PreparedHud; preview: Blob;
}): FormData {
