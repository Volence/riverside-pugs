...
grid, and a link flow. All of these are state-heavy UI that string-concatenated
...
1. **The nav token** (`app.js:38`, `let nav = 0`). Every route bumps it; async
   ...
   abort/ignore in the fetch hook, and the bug it prevents is the same bug.
...
backend is already TS. No CSS framework, just hand-written CSS with custom properties,
...
is where people idle before being yanked into a 30-second ready check, and a design
...
Rejected: "Broadcast", meaning cool blue-black, cyan accent, and chamfered
translucent panels (the Faceit/Valorant look). Interchangeable with every other game's stat site,
...
one file, not a grep. That is the hedge against a taste call made overnight.
...
Frontend tests are deliberately shallow, because this is presentation code and
testing markup is how you get a suite that breaks on every design change:
