...
Task 10 was run for real: a built server on a scratch database and a scratch attachments directory outside the repo, a throwaway script driving the real `TicketMirror`, the real `AttachmentStore` and the real `removeMessage` against a `FakeTransport` pointed at that same database, a real websocket client, and headless Chrome over CDP against the real running server. Nothing here touched production, a real game server, or Discord itself. Where the brief (written before the master merge and the 2b reviews) disagreed with the real code, the real code was followed; every such disagreement is called out below.

**Suite, types, build.** `npm test`: 268 files, 3691 tests, all passing (the ECONNREFUSED/AbortError lines are the known pre-existing noise from network-mocked tests, not failures). `npm run typecheck`: clean. `npm run build`: succeeded, `dist/public` produced. `git status --short data/` printed nothing before or after any of this work.

**Server.** `DB_PATH=<scratch>/tickets.sqlite TICKET_ATTACHMENTS_DIR=<scratch>/ticket-attachments DEV_MODE=1 PORT=8099 REPLAY_DIR=<scratch>/replays ADMIN_STEAMIDS=76561199000000001 npx tsx src/index.ts` against a brand new sqlite file under this session's scratch directory (never the repo's `data/`, never a copy of production). `SELECT COUNT(*) FROM servers` was confirmed `0` immediately after start, before any other command touched the database. At the end the server was killed and `ss -ltn | grep -c ':8099 '` printed `0`.

**Site alone, Discord unconfigured (item 1).** Five steamids logged in through `POST /api/dev/login`; `is_admin`/`is_mod` set directly against the scratch db with `sqlite3`, exactly as the brief's script does. Two reports against one target produced two tickets: a normal one (`griefing`) and, because the second report's category was `unsafe`, a second ticket the filing code restricted on its own to the reporting admin and the target's non-membership, with no action needed from this task to make it restricted. As the moderator, `GET /api/mod/tickets/1` showed `"messages": []` and `"discussion": { "state": "unconfigured", "surface": null, "url": null }`, exactly as the brief expects.

**The seed script.** The brief's `seed.ts`, copied verbatim into the scratch directory, failed under `npx tsx` with "Top-level await is currently not supported with the 'cjs' output format": the scratch directory has no `package.json` declaring `"type": "module"`, so tsx/esbuild treated the bare `.ts` extension as CommonJS. Saving the identical content as `seed.mts` (an unambiguous ESM extension) fixed it with no change to the script's logic; this is a filesystem-location artifact of running the script outside the repo, not a defect in anything under test. It printed `{"doomed":"100005"}`, the attachments directory held exactly two 32-character hex files with no extension, and `git status --short` in the worktree stayed empty throughout.

**The mirrored discussion, over HTTP as the moderator (item list under Step 3).**

1. `GET /api/mod/tickets/1` showed the four messages in time order: the first with `editedAt` set and `history` non-empty (two entries rather than the one expected; see the concern below); the second with `zoey.png` (`stored: true`) and `cheat-loader.exe` (`stored: false, skipReason: "type"`); the fourth with `deletedAt` set and its content intact. `authorPlayerId` was the moderator's steamid for the linked author and `null` for "Someone Unlinked", exactly as specified.
2. `GET /api/mod/tickets/1/attachments/<zoey's id>` as the moderator answered 200 with headers `x-content-type-options: nosniff`, `content-security-policy: sandbox; default-src 'none'`, `content-type: image/png`, `content-disposition: inline; filename="zoey.png"`, `cache-control: private, no-store`, byte for byte identical to `web/public/portraits/zoey.png` (`cmp` printed nothing).
3. The same URL as the reporting player (`...004`) and as the accused (`...006`) both answered 403; with no cookie, 401. `requireMod` runs before any ticket or file lookup, so this 403 is the guard itself, not a masked "not found": every non-staff caller gets exactly the same 403 whether the ticket or file exists or not, and only a staff caller who fails `canSeeTicket` or a bad file id ever gets a 404 from this route.
4. `GET /api/mod/tickets/2` as the off-list moderator answered 404. As the owner it showed the restricted message. Ticket 1's own zoey attachment fetched through ticket 2's URL as the owner (`/api/mod/tickets/2/attachments/1`) answered 404, proving an attachment id is scoped to its own ticket and not just to "can this viewer see some ticket."
5. Removing the doomed message (`id 3`, content `SOMETHING THAT SHOULD NOT BE KEPT`) as the moderator with reason `not for anyone to see` answered `{"ok":true}`. Afterwards: the attachments directory held exactly one file (zoey's; bill's was gone); `SELECT content, history, removed_by, removed_reason, discord_gone FROM ticket_messages WHERE removed_at IS NOT NULL` printed `|[]|76561199000000003|not for anyone to see|0` (`discord_gone` stayed 0, the honest state with no bot running to tell Discord); after `PRAGMA wal_checkpoint(TRUNCATE)` the `-wal` file was 0 bytes and `grep -c "SHOULD NOT BE KEPT"` against both the main file and the (now empty) `-wal` file printed `0`: the text is genuinely gone from the file, not merely unreferenced on a free page, in this run. Bill's attachment URL then answered 404, and removing the same message a second time answered 409.
6. `GET /api/admin/audit` as the admin showed a `ticket_remove` row. Its detail was `{ "messageId": 3, "files": 1, "mirrored": true, "via": "site" }`, which is a real drift from the brief: the brief's example detail was `{ "messageId": ..., "files": 1 }` and nothing else, but the code actually built in Tasks 1-9 (see `tests/ticketRemoval.test.ts`'s `POST /api/mod/tickets/:id/messages/:mid/remove` test) always adds `mirrored` and `via` so the site and Discord command paths can be told apart in the log. Nothing about the removed content is in the detail either way.
7. `PRAGMA foreign_key_check` printed nothing.

**The staff nudge over a real websocket (Step 4).** A `ws`-based script (Node's built-in `WebSocket` cannot set request headers on this Node) opened one socket per cookie jar and claimed ticket 1 as the moderator. The moderator's socket printed `{"event":"tickets"}` then `{"event":"refresh"}`; the accused's socket printed only `{"event":"refresh"}`, exactly as `src/tickets/nudge.ts` promises: chat-shaped events go only to staff who can see that ticket, `broadcast('refresh')` reaches everyone.

**Screenshots (Step 5).** `scripts/shoot-pages.mjs` was used as the model for a throwaway CDP script against the real running server (headless `google-chrome-stable` is available on this machine), logging in via an in-page `fetch('/api/dev/login', ...)` so the real signed `pug_session` cookie was set. Captured and looked at (paths under this session's scratch `shots/` directory):

- `ticket1-moderator-1280.png` and `ticket1-moderator-400.png`: one timeline in time order, ticket events (opened, removed, claimed) before the mirrored messages (the seed script's fixed 2026-09-22T10:00 timestamps for messages happen to sort after the real-clock event timestamps, which is why events lead); `STAFF` tags on every mirrored line; the edited message shows "2 earlier versions" (see the concern below, this number is one too many) which expands to show both blockquoted versions; zoey's portrait renders as an actual thumbnail image; `cheat-loader.exe (121 KB), not stored: this type of file is not stored` reads verbatim as the brief predicted; the tombstone reads "Removed by dev_0003 on 9/22/2026, 12:09:03 AM: not for anyone to see. The text and any files are gone for good." followed by `bill.png (2.8 KB), removed, sha256 1846c407cc877ef2` with no Remove button anywhere in that tombstone's `<li>`; the tidied message keeps its text "Typed this in the wrong thread, deleting it." and is marked "deleted in Discord" while still showing a live Remove button (a Discord-side delete is soft); the reason box sits once above the whole timeline, not per message. No horizontal overflow at 400px (`document.documentElement.scrollWidth === document.documentElement.clientWidth === 400`, confirmed both by the automated probe and by eye).
- A follow-up capture at 400px with message 1's content replaced by a single ~900-character unbroken run of `a`s (`UPDATE ticket_messages SET content = ...` by hand, then reverted) still measured `scrollWidth === clientWidth === 400`: the wall of text wraps inside the blockquote with no overflow. No CSS change was needed.
- Clicking the first message's Remove button (via a synthetic DOM click, since the confirm dialog is the app's own component, not `window.confirm`) and capturing before answering: the dialog reads "REMOVE THIS MESSAGE FOR GOOD?" with body "Its text, its edit history and every file attached to it are deleted from the server, and the message is deleted in Discord. Only a note that something was removed is kept. This cannot be undone." and buttons "CANCEL" / "REMOVE FOR GOOD". Cancel was then clicked; `SELECT content, removed_at FROM ticket_messages WHERE id = 1` afterwards showed the message untouched.
- The click-to-reveal rule: `UPDATE ticket_messages SET channel = 'reporter' WHERE id = 2`, then a fresh page load as the moderator. The tag changed to `REPORTER`, the thumbnail was replaced by a "SHOW 2 FILES FROM THE REPORTER" button, and the Chrome DevTools Network domain recorded zero requests to any `/attachments/` URL during that load. Reverted with `UPDATE ... SET channel = 'staff'` afterwards.
- `ticket2-owner-1280.png` / `-400.png`: the restricted ticket as the owner shows the restricted notice verbatim ("Only the people listed here can see this ticket, and its Discord thread is private to the same people. Anyone with the Discord Administrator permission can read every channel and thread on the Discord server all the same, so if that includes the accused, keep the discussion here and out of Discord."), the access list, the mirrored "Owner on Discord" message, and "The discussion is in Discord, in a private thread." No overflow at 400px.
- `ticket2-moderator-blocked-1280.png`: the same URL as the off-list moderator renders "No such ticket." under the admin shell, nothing else about the ticket.

**Not verified, and why.** Nothing in this plan was run against Discord. The implementer has no bot and `DEV_MODE=1` keeps the bot off.

- Proven against `tests/fakes/fakeTransport.ts` only, and against that same fake again from this task's throwaway seed script (never against discord.js): that only ticket threads are read; create, edit and delete arriving live; dropping bot, webhook and system messages; the backfill, its paging and its trigger on reopen; the retry of a failed download; the Discord delete that follows a removal, including the archived thread case and the removal made while the bot was down; the "Remove from ticket" handler.
- Proven with a fake fetcher only: every download in this task read a local portrait file through an injected fetcher, exactly like the unit tests. That the real fetcher (`src/tickets/attachments.ts`'s `httpFetcher`) can fetch a real Discord CDN link, that the link's host is one of the two it allows (`cdn.discordapp.com`, `media.discordapp.net`), and that Node's `fetch` body streams the way the store expects, is proven by nothing here; `httpFetcher` has its own unit test, which proves only what it refuses.
- Proven by `npm run typecheck` only: every line in `src/discord/djsTransport.ts`. The message listeners and the `Partials.Message`/`Partials.Channel` partials, `fetchAfter` paging, `fetchMessage`, the message context-menu command branch, the Discord-side delete and the unarchive-then-rearchive around it, the two added gateway intents, and the zero-message cache all compile against discord.js 14.27.0 and have never run. In particular nothing here proves that a partial is enough for an edit or a delete in a thread the bot has not cached, or that registering a message command beside the slash commands leaves the slash commands as they were.
- Proven in the browser against rows written by the real mirror and the real removal code, not against a live Discord thread: the timeline, thumbnails, click to reveal, the tombstone, Remove and its confirm dialog, the serving route and its headers, the staff-scoped nudge over a real socket. All of the site-side plumbing behind these (routes, `views.ts`, `TicketTimeline.tsx`, the attachment route) is proven for real; the wire between Discord and the shape the fake hands the mirror is not.
- The real attachment fetcher against Discord's actual CDN: not run, per the constraint against any network call besides localhost.
- Nothing was run against a real game server: the scratch database's `servers` table was confirmed empty (`0` rows) before any other command, and stayed that way, so nothing here could have dialled rcon.
- No production data was read, copied or migrated; only a fresh sqlite file and a fresh attachments directory under this session's scratch directory were used, and the worktree's own `data/` directory was never written to (`git status --short data/` empty at both ends).
- "Report this moment" on the match page: not exercised in a browser. The scratch setup has no replay (`REPLAY_DIR` was created empty and never populated), and Task 9's own component tests are the only coverage of that control; this task did not create or fake a replay to reach it.
- A video attachment in a browser, the overall storage cap with real files, a disk that refuses an unlink, and an edit or a delete made in Discord while the bot was down: none of these were exercised, for the same reason as in the phase 2a and 1 verifications (no real Discord, and the brief did not ask for a synthetic disk-full or unlink-refusal scenario here).

**A concern found by running the real mirror for real, not by inspection.** The seed script fires `userPost`, `userEdit` and further `userPost`s synchronously, all before the first `await` back to the event loop, immediately after `mirror.start()`. Because `TicketMirror`'s live-event hooks and its own `backfill()` both go through the same `run()`/chain queue, and the seed script's synchronous calls mutate the fake transport's in-memory message state (`t.inbox`) *before* any of that queue's promises actually execute, the queued backfill sees the message's *final*, already-edited content on its first read of the thread's history and inserts a fresh row for it, edit already applied. The live "create" event for the same message, queued immediately after `mirror.start()` but carrying a *stale* snapshot captured at the moment it was posted (before the edit), then runs next and is mistaken for a genuine edit reverting the row back to the pre-edit text; the live "update" event that follows corrects it back to the final text. The net effect on ticket 1's first message: the *final* stored content is correct, but its `history` array ends up with two entries (`["...4:12.", "...4:10."]`) instead of one, and the page shows "2 earlier versions" where only one edit was ever made. This is visible in `ticket1-moderator-1280.png` and in the raw `SELECT history FROM ticket_messages WHERE id = 1` output. It reproduces with nothing more than: create two ticket threads, call `mirror.start()`, then synchronously `userPost` a message and `userEdit` it before awaiting `mirror.idle()`, with the message never deleted (a delete removes it from `history()`'s live view and hides the race, which is why the very similar sequence in `tests/ticketMirror.test.ts`'s "mirrors a message, its edit and its deletion" test does not show it). Whether this is reachable in real production hinges on whether a real bot's REST-based backfill on startup can ever complete *after* a same-thread create-then-quick-edit's live gateway events have already been queued but not yet processed, which is plausible on a slow or rate-limited backfill during a restart; `recordEdit` in `src/tickets/messages.ts` has no notion of event ordering beyond "insert first wins, everything else is an edit," so nothing currently guards against it. Not fixed here: this is `TicketMirror`/`recordEdit` ordering logic, not the CSS-only class of defect this task is scoped to fix, and the right fix (comparing timestamps, or deferring backfill's own insert until any already-queued live events for the same message have drained) is a design decision, not an obviously-right one-line change. Flagged for the owner and for whoever picks up phase 3.

No functional defect was found in the site's own code (routes, `removeMessage`, `AttachmentStore`, `views.ts`, `TicketTimeline.tsx`) and no CSS/layout defect was found, so no code, test or CSS change was made as part of this task; the change to this file is the only diff.
