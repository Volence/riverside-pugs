...
Task 9 was run for real: a built server on a scratch database outside the repo, a throwaway script driving the real `TicketSync`, `handleTicketButton` and `handleTicketModal` against a `FakeTransport` pointed at that same database, and headless Chrome over CDP against the real running server. Nothing here touched production, a real game server, or Discord itself.

**Suite, types, build.** `npm test`: 218 files, 3055 tests, all passing (the ECONNREFUSED/AbortError lines are the known pre-existing noise from network-mocked tests, not failures). `npm run typecheck`: clean. `npm run build`: succeeded, `dist/public` produced.

**Server.** `DEV_MODE=1 PORT=8099 DB_PATH=<scratch>/tickets.sqlite REPLAY_DIR=<scratch>/replays ADMIN_STEAMIDS=76561199000000001 npx tsx src/index.ts` against a brand-new sqlite file under the harness's own scratch directory (never the repo's `data/`, never a copy of production). `SELECT COUNT(*) FROM servers` was confirmed `0` immediately after every start, including two restarts made necessary by a scratch-server process outliving an earlier `pkill` (its real cmdline held `src/index.ts` but not the literal substring the kill pattern matched; found by `ss -ltnp` naming the surviving pid, killed by pid, re-verified `0` rows before continuing). Made-up ids only: `fake-forum-1` and `fake-channel-1` stood in for a real Discord forum/channel snowflake everywhere, never anything resembling the project's actual channel ids.

**Site alone, Discord unconfigured (item 1).** Seven steamids logged in through `POST /api/dev/login`; `is_admin`/`is_mod` set directly against the scratch db since that route grants neither. Four reports filed: griefing and toxicity about one target (one normal ticket, 2 reporters), a matching `unsafe` report about the same target (a second, restricted ticket, seeded with the one configured admin), and a `cheating` report about a second target (a third, normal ticket). As the moderator, `GET /api/mod/tickets?filter=open` returned `counts: { open: 2, mine: 0, closed: 0 }` and only the two normal tickets (the restricted one absent); as the owner, `counts.open` was `3`. Claiming ticket 1 as the moderator moved `mine` to `1`. `GET /api/mod/tickets/1` showed `discussion: { state: "unconfigured", surface: null, url: null }`. Setting `discord_tickets_forum_id` to a made-up id through `PUT /api/admin/settings/discord_tickets_forum_id` flipped it to `{ state: "pending", surface: "forum", url: null }` with the bot off, exactly the honest answer the plan calls for; blanking the setting again put it back to `unconfigured`. Merging the second target into the first (`POST /api/admin/players/.../merge`) folded its ticket into ticket 1: `reports` went from 2 to 3, an event of kind `folded` carried `detail.from` naming the ticket that used to be about the merged-away account, `PRAGMA foreign_key_check` printed nothing, `GET /api/admin/audit` held no row pointing at the now-gone ticket id, `SELECT COUNT(*) FROM ticket_threads` was `0`, and `SELECT COUNT(*) FROM ticket_reports WHERE announced_at IS NULL` equalled `4`, the number of reports filed: nothing had been said anywhere, because nothing that says it was running.

**Discord side, against the real TicketSync/FakeTransport (items 2-7).** A throwaway script (kept outside the repo, under the scratch directory) opened the same sqlite file with `openDb`, set `discord_tickets_forum_id`/`discord_tickets_channel_id` to the made-up ids directly in the settings table, linked fake Discord ids onto the relevant players with the real `linkDiscord`, and built a real `TicketSync` over a real `FakeTransport`. It never called `sync.start()`: doing so once during development also subscribed the sync to `publishTicketSignal`, and since the script calls `claimTicket`/`closeTicket` directly (via the button handlers) in the same process, those actions publish a signal of their own (`src/tickets/actions.ts`'s `told()`), which raced the script's own explicit `reconcile()` calls and once surfaced a benign, self-healing "Thread is archived" from two overlapping reconciles touching one thread. That race is an artifact of this script calling `reconcile()` both ways in one process; in production only the signal queue ever calls it. Calling `reconcile()` directly throughout avoided it, and every reconcile in the results below is still the real `reconcile()` method.

- A normal ticket got exactly one forum post: a case card naming the accused (`Accused` field, markdown-escaped) and no reporter, tags `[claimed, griefing, toxicity, cheating]` (it was already claimed from the HTTP step above), and buttons `t:<id>:claim=Release`, `t:<id>:close=Close`, plus a link, in that order.
- The restricted (`unsafe`) ticket got no forum post at all: a private thread on the made-up text channel, named just `Ticket #2`, whose members were exactly its access list's linked Discord id (one person: the configured admin), one DM sent to that same person, and nothing at all published to the admin feed.
- A second report on the normal ticket, filed by a fresh reporter over HTTP, produced exactly one "Another report" line in the existing thread (naming the category, not the reporter) and no second forum post; the card was refreshed (its hash changed) once. Reconciling twice more after that made zero further edits and sent zero further messages; the card hash was stable.
- A fresh normal ticket (a new target, deliberately with no restricted sibling, to exercise the direct restrict path rather than the fold path) was promoted to moderator through the site's real `POST /api/admin/players/:steamid/mod` route. With the fake's thread-delete forced to fail on **both** attempts a single pass makes for one forbidden post (the reconciler's own global sweep, then its per-ticket pass; failing only one is silently absorbed by the other, which is why `tests/ticketSyncAccess.test.ts` itself uses two failures for this same reason), the forum post survived that pass and the promoted account stayed out of the forum's access set. The next reconcile deleted the post, created a private thread in its place, and only then let the account into the forum access set.
- Closing the normal ticket over HTTP as the moderator, then reconciling, ended the thread locked and archived with tags including `closed`, and the private note never reached the payload sent to "Discord". Reopening unlocked and unarchived it; its status tag came back as `claimed`, not `open`, because closing and reopening never clear `claimed_by` and the ticket had been claimed since the very first HTTP claim in Step 3 - the honest state, not a bug.
- Pressing the real button handlers directly (`handleTicketButton`/`handleTicketModal`, no discord.js involved): an off-list moderator pressing Claim or Close on the restricted ticket got back `"No such ticket."` both times, with no `.modal` on the reply and nothing in the reply naming the ticket or the accused; a plain player (no staff flag) pressing Claim on the normal ticket got `"Staff only."`, also with no modal. A real moderator's press worked end to end: release, claim, Close (which returned the real modal definition, an `outcome` select plus a `note` field), and submitting that modal closed the ticket for real and (after a reconcile) left its thread locked.
- The admin-feed rule: a new normal report with the forum id blank produced exactly one `report` admin-feed event; the same with the forum id set produced zero; an `unsafe` report (which seeds a restricted ticket and marks its own row `feed_held = 1` at insert time) whose ticket was afterwards forced back to normal with a direct `UPDATE tickets SET restricted = 0` still produced zero feed events on the next reconcile even with the forum blank, and the report row's `feed_held` column read back `1`: the hold is permanent, not tied to the ticket's current restriction state.

**Browser (item 8).** A throwaway CDP script adapted from `scripts/shoot-pages.mjs` (kept outside the repo) logged in via an in-page `fetch('/api/dev/login', ...)` against the real running server so the real signed session cookie was set, then captured at 1280px and 400px: the normal ticket with a real thread on it (as the moderator), a second normal ticket whose discussion had never been touched by Discord (as the moderator, forum id blank at capture time - used in place of toggling the same ticket's discussion state back and forth, since by then ticket 1 already had a thread; both are genuinely normal tickets, one simply predates any Discord configuration), and the restricted ticket (as the owner, who is on its access list). All six images were opened and looked at:
  - The discussion line rendered on every capture: "The discussion is in Discord." (ticket with a thread), "Discord discussion is not configured. An admin can set the tickets forum and the tickets channel in Settings; until then this ticket is worked here." (the unconfigured one), and "The discussion is in Discord, in a private thread." (the restricted one).
  - The restricted ticket's notice says, verbatim: "Only the people listed here can see this ticket, and its Discord thread is private to the same people. Anyone with the Discord Administrator permission can read every channel and thread on the Discord server all the same, so if that includes the accused, keep the discussion here and out of Discord." - a plain statement of the Administrator-permission rule, present at both widths.
  - The automated `document.documentElement.scrollWidth <= clientWidth` probe found no overflow on any of the three 400px captures (`scrollWidth === clientWidth === 400` in every case), and manual inspection agreed: every field, the timeline, and the restricted notice wrap cleanly. The admin sub-tab strip (Players/Matches/Tickets/...) is visually clipped at 400px in the restricted-ticket capture, but that is `.tabs { overflow-x: auto }` in `web/src/styles/app.css`, an existing, deliberate, site-wide scrollable tab strip used by every admin sub-page since before this phase, not something this phase's work touched or regressed. No CSS or code changes were needed.

**Not verified, and why.** Nothing in this plan was run against Discord. The implementer has no bot and `DEV_MODE=1` keeps the bot off.

- Proven against `tests/fakes/fakeTransport.ts` only, and against the same fake again from the throwaway script above (never against discord.js): creating a forum post and its case card; tags; the "Another report" line; the card refresh and its hash; lock and archive on close, and the reverse on reopen; remaking a post deleted by hand; deleting a forum post that must not exist; retiring a folded thread; a restricted ticket's private thread and its membership; the access list DM; the forum's permission overwrites; the admin feed line and the rule that silences it.
- Proven as plain functions only, with no Discord interaction behind them: Claim, Release, Close and the Close modal's submit (`src/discord/ticketButtons.ts`). That a press on a real Discord button reaches them, that Discord actually opens the modal `i.showModal(...)` is asked to open, and that its select comes back through `i.fields.getStringSelectValues(...)` as `fields.outcome`, is proven by nothing: all of that lives in `src/discord/djsTransport.ts`, which this task never ran.
- Proven by `npm run typecheck` only: every line of `src/discord/djsTransport.ts` added in Task 3. It compiles against discord.js 14.27.0. It has never run.
- Not exercised at all: a ticket about staff that could not be restricted (`about_staff` on the page); the reconciler's five minute timer (`intervalMs: 0` was used throughout, as the existing tests do); two processes reconciling the same database at once (only sequential passes were run); a real forum's five tag limit and twenty tag ceiling; Discord rate limits on a first start with many open tickets; the Settings page's "Tickets forum channel id"/"Tickets text channel id" fields in the browser (their labels and help text were read from `src/settingsSchema.ts` and the setting itself was exercised over HTTP in item 1, but the rendered Settings tab was not screenshotted).
- No production data was read, copied, or migrated; only a fresh sqlite file under the harness's scratch directory was used, and the worktree's own `data/` directory was never written to. No real Discord channel or guild id (the project's actual forum `1551668183906394204`, text channel `1551668103199461497`, or guild `1539966341543633117`) appeared anywhere in any command, script, or file this task touched; `fake-forum-1` and `fake-channel-1` stood in for them throughout.
