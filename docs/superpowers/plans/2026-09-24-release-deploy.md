...
2. **Seeding commit** in the deploy repo: create `boxes/{dallas,chicago,riverside-3,riverside-4}/left4dead/cfg/{local.cfg,server.cfg}` and `boxes/<slug>/left4dead/addons/sourcemod/configs/hostname/server_hostname.txt` from each box's live copy (read-only fetch as in the fleet check). Before committing, strip every `rcon_password` and `sv_password` line out of each seeded `server.cfg` (and any other seeded cfg) and put those lines in that box's own `secrets.cfg` on the box first, so the box still gets them; staging refuses a commit whose cfgs set either one (a `tv_password` is only warned about). Then `git mv overrides/left4dead/cfg/local.cfg boxes/dallas/left4dead/cfg/local.cfg` only if equal to Dallas's live copy, commit, push.
