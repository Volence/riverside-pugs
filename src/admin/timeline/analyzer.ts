...
/** A hidden_track clip's score is the lag-tolerant one (the lag search's own
 *  score), not a straight fidelity, so it is labelled differently. When the
 *  detail carries the lag that won, name it too. */
function fidelityText(r: Row): string {
  if (r.kind !== 'hidden_track') return `fidelity ${r.score.toFixed(2)}`;
  let lagMs: unknown;
  try {
    lagMs = (JSON.parse(r.detail) as { lagMs?: unknown }).lagMs;
  } catch {
    lagMs = undefined;
  }
  const suffix = typeof lagMs === 'number' ? ` at ${lagMs} ms lag` : '';
  return `lag-tolerant fidelity ${r.score.toFixed(2)}${suffix}`;
}
...
export const analyzerAdapter: TimelineAdapter = {
  ...
  items({ db, ids }): TimelineItem[] {
    const rows = db.prepare(
      `SELECT c.id, c.match_id, c.ordinal, c.half, c.start_ms, c.end_ms, c.kind, c.score, c.detail,
    ...
    return rows.map((r) => ({
      ...
      kind: r.kind,
      summary: `Analyzer clip: ${CLIP_TEXT[r.kind] ?? r.kind}, ${fidelityText(r)} over `
  ...
  evidence(db: DB) {
    return (db.prepare(
      // hidden_track is uncalibrated (spec section 6), so it does not push a
      // file into the review queue on its own. It still lists on the
      // player's file via items() above. Lift this exclusion once it is
      // calibrated.
      `SELECT c.steamid, MAX(COALESCE(m.ended_at, m.created_at)) AS at
       FROM integrity_clips c JOIN matches m ON m.id = c.match_id
       WHERE c.analyzer_version = ? AND c.kind <> 'hidden_track' GROUP BY c.steamid`,
