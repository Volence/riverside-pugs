...
describe('ModCallPoster', () => {
  ...
  it('names the caller, the staff member and the details when the call is about staff', async () => {
    ...
    const p = inAdmin()[0].payload;
    const body = JSON.stringify(p);
    expect(body).not.toContain('About a staff member');
    expect(body).toContain('Caller: **player0** (<@900>)');
    expect(body).toContain('About: **player7** (<@907>)');
    expect(body).toContain('<@901>');
    expect(body).toContain('secret details');
    expect(body).toContain('second secret');
    expect(body).toContain(`/admin/people/tickets/${first.ticket_id}`);
    expect(p.components[0].map((b) => b.label)).toContain('Ticket');
    // Named, never pinged: only the role is.
    expect(p.mentionUserIds).toEqual([]);
    expect(p.content).toBe(`<@&${ROLE}>`);
  });
