...
describe('ReleaseEngine', () => {
  ...
  it('SCRATCH recover then boot reconcile', async () => {
    const id = stage();
    db.prepare("UPDATE releases SET state = 'deploying' WHERE id = ?").run(id);
    db.prepare("UPDATE release_boxes SET state = 'written' WHERE release_id = ? AND server_id = ?").run(id, s1);
    db.prepare("UPDATE release_boxes SET state = 'skipped' WHERE release_id = ? AND server_id = ?").run(id, s2);
    db.prepare("UPDATE servers SET status = 'offline' WHERE id = ?").run(s1);
    const e = engine({ humans: async () => { await new Promise((r) => setTimeout(r, 5)); return 0; } });
    e.recover();
    const { reconcileServers } = await import('../src/serverRelease.js');
    const stranded = reconcileServers(db, { release: (sid: number) => db.prepare("UPDATE servers SET status='idle' WHERE id=?").run(sid) } as any);
    await e.settled();
    expect(JSON.stringify({ stranded, restarts, box: boxState(id, s1), status: getServer(db, s1)!.status })).toBe('show');
  });
});
